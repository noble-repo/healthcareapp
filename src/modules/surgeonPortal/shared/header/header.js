...
function Header(props) {
    ...
    function getPatientListPath() {
        return ((props?.userDetails?.role === Auth0RoleNameConstants.ANAESTHESIOLOGIST || props?.userDetails?.role === Auth0RoleNameConstants.SUB_ANAESTHESIOLOGIST) ? '/anaesthesiologist/patients-list' : '/surgeon/patients-list')
    }

    function getUserListPath() {
        return ((props?.userDetails?.role === Auth0RoleNameConstants.ANAESTHESIOLOGIST || props?.userDetails?.role === Auth0RoleNameConstants.SUB_ANAESTHESIOLOGIST) ? '/anaesthesiologist/list-of-users' : '/surgeon/list-of-users')
    }

    function getCalendarPath() {
        return ((props?.userDetails?.role === Auth0RoleNameConstants.ANAESTHESIOLOGIST || props?.userDetails?.role === Auth0RoleNameConstants.SUB_ANAESTHESIOLOGIST) ? '/anaesthesiologist/calendar' : '/surgeon/calendar')
    }
    const goToMeeting=()=> {
        var url = 'https://h3a-vchat.friendz.today/r/617X04w4ra6004x16/' + props?.surgery?.selectedSurgery?.virtualMeeting?.startTime;
        window.location.href= url;
    }
    const date1 = moment(props?.surgery?.selectedSurgery?.virtualMeeting?.startTime);
    const date2 = moment();
            ...
            <AppBar color="white" className={classes.appBar}>
                <Toolbar className={classes.toolbar}>
                    ...
                    {props.tabName === "patients" ? (
                        <a className={classes.active} onClick={() => history.push(getPatientListPath())}>
                            PATIENTS
                        </a>
                    ) : (
                        <a className={classes.inactive} onClick={() => history.push(getPatientListPath())}>
                            PATIENTS
                        </a>
                    )}

                    {props.tabName === "calendar" ? (
                        <a className={classes.active} onClick={() => history.push(getCalendarPath())}>
                            CALENDAR
                        </a>
                    ) : (
                        <a className={classes.inactive} onClick={() => history.push(getCalendarPath())}>
                            CALENDAR
                        </a>
                    )}

                    {props.tabName === "users" ? (
                        <a className={classes.active} onClick={() => history.push(getUserListPath())}>
                            USERS
                        </a>
                    ) : (
                        <a className={classes.inactive} onClick={() => history.push(getUserListPath())}>
                            USERS
                        </a>
                    )}

                    {props.tabName === "billing" ? (
                        <a className={classes.active} onClick={() => history.push('/surgeon/billing-details')}>
                            BILLING
                        </a>
                    ) : (
                        <a className={classes.inactive}
                           onClick={() => history.push('/surgeon/billing-details')}>
                            BILLING
                        </a>
                    )}
                    {props.tabName === "videos" ? (
                        <a className={classes.active} onClick={() => history.push('/surgeon/video-screen')}>
                            VIDEOS
                        </a>
                    ) : (
                        <a className={classes.inactive} onClick={() => history.push('/surgeon/video-screen')}>
                            VIDEOS
                        </a>
                    )}
                    ...
                    <Row className={classes.rowfixing}>
                        {/* <div>
                           {diff >= 10 &&<button onClick={goToMeeting} className="btn btn-success" disabled>Upcoming Meeting</button>}
                           {diff <= 10 &&<button onClick={goToMeeting} className="btn btn-success">Upcoming Meeting</button>}
                        </div> */}
                        <div>
                            <img
